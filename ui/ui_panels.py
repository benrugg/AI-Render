    ...
    def draw(self, context):
        ...
        if not utils.is_installation_valid():
            ...
            utils.label_multiline(
                layout,
                text="Enable AI Render in this scene to start using Stable Diffusion",
                alignment="CENTER",
            )
...
class AIR_PT_setup(bpy.types.Panel):
    ...
    def is_api_key_valid(cls, context):
        return (
            utils.get_dream_studio_api_key(context) != ""
            and context.scene.air_props.error_key != "api_key"
        )

    @classmethod
    def has_dimensions_issue(cls, context):
        return (
            not AIR_PT_setup.are_dimensions_valid(context)
            or not AIR_PT_setup.are_dimensions_small_enough(context)
            or not AIR_PT_setup.are_dimensions_large_enough(context)
        )

    @classmethod
    def are_dimensions_valid(cls, context):
        return (
            utils.are_dimensions_valid(context.scene)
            and context.scene.air_props.error_key != "invalid_dimensions"
        )

    @classmethod
    def are_dimensions_small_enough(cls, context):
        return (
            not utils.are_dimensions_too_large(context.scene)
            and context.scene.air_props.error_key != "dimensions_too_large"
        )

    @classmethod
    def are_dimensions_large_enough(cls, context):
        return (
            not utils.are_dimensions_too_small(context.scene)
            and context.scene.air_props.error_key != "dimensions_too_small"
        )
    ...
    def draw(self, context):
        ...
        if (
            not AIR_PT_setup.is_api_key_valid(context)
            and utils.sd_backend(context) == "dreamstudio"
        ):

            utils.label_multiline(
                layout,
                text="Setup is quick and easy. No downloads or installation. Just register for a DreamStudio API Key.",
                icon="INFO",
                width=width_guess,
            )
            ...
            col = row.column()
            col.operator(
                operators.AIR_OT_setup_instructions_popup.bl_idname,
                text="Instructions",
                icon="HELP",
            )

            row = layout.row()
            row.operator("wm.url_open", text="Watch Full Tutorial", icon="URL").url = (
                config.VIDEO_TUTORIAL_URL
            )

            row = layout.row()
            row.operator(
                "wm.url_open", text="Sign Up For DreamStudio (free)", icon="URL"
            ).url = config.DREAM_STUDIO_URL
        ...
        # show the image dimension help if the dimensions are invalid or too large
            ...
            if not AIR_PT_setup.are_dimensions_valid(context):
                utils.label_multiline(
                    layout,
                    text="Adjust Image Size: \nStable Diffusion only works on certain image dimensions.",
                    icon="INFO",
                    width=width_guess,
                )
                ...
                utils.label_multiline(
                    layout,
                    text=f"Adjust Image Size: \nImage dimensions are too large. Please decrease width and/or height. Total pixel area must be at most {round(utils.get_active_backend().max_image_size() / (1024*1024), 1)} megapixels.",
                    icon="INFO",
                    width=width_guess,
                )
            else:
                utils.label_multiline(
                    layout,
                    text=f"Adjust Image Size: \nImage dimensions are too small. Please increase width and/or height. Total pixel area must be at least {round(utils.get_active_backend().min_image_size() / (1024*1024), 1)} megapixels.",
                    icon="INFO",
                    width=width_guess,
                )
            ...
            if utils.is_using_sdxl_1024_model(scene):
                col.operator(
                    operators.AIR_OT_show_dimension_options_for_sdxl_1024.bl_idname,
                    text="Other",
                )
            else:
                col.operator(
                    operators.AIR_OT_show_other_dimension_options.bl_idname,
                    text="Other",
                )

            if (
                utils.get_active_backend().supports_upscaling()
                and props.do_upscale_automatically
            ):
                layout.separator()
                box = layout.box()
                utils.label_multiline(
                    box,
                    text=f"Final image will be upscaled {round(props.upscale_factor)}x larger than these initial dimensions.",
                    width=width_guess - 20,
                )
        ...
        # else, show the ready / getting started message and disable and change image size buttons
            ...
            utils.label_multiline(
                layout,
                text="You're ready to start rendering!",
                width=width_guess,
                alignment="CENTER",
            )
            row = layout.row()
            row.operator("wm.url_open", text="Help Getting Started", icon="URL").url = (
                config.VIDEO_TUTORIAL_URL
            )
...
class AIR_PT_prompt(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = box.row()
        label = (
            "Apply a Preset Style (to All Prompts)"
            if props.use_animated_prompts
            else "Apply a Preset Style"
        )
        ...
        if props.use_preset:
            ...
            row.template_icon_view(
                props, "preset_style", show_labels=True, scale_popup=7.7
            )
            ...
            col = row.column()
            col.operator(
                operators.AIR_OT_copy_preset_text.bl_idname, text="", icon="COPYDOWN"
            )
...
class AIR_PT_controlnet(bpy.types.Panel):
    ...
    def poll(cls, context):
        return (
            ...
            and context.scene.air_props.is_enabled
            and utils.sd_backend(context) == "automatic1111"
        )
    ...
    def draw(self, context):
        ...
        if not props.controlnet_close_help:
            ...
            utils.label_multiline(
                box,
                text="ControlNet is an extension for Automatic1111 that provides a spectacular ability to match scene details - layout, objects, poses - while recreating the scene in Stable Diffusion. It can also create much more stable animations than standard Stable Diffusion.",
                width=width_guess,
            )

            row = box.row()
            row.operator("wm.url_open", text="Learn More", icon="URL").url = (
                config.HELP_WITH_CONTROLNET_URL
            )
        ...
        if not props.controlnet_available_models:
            row = layout.row()
            row.operator(
                operators.AIR_OT_automatic1111_load_controlnet_models_and_modules.bl_idname,
                text="Load Models from Automatic1111",
                icon="FILE_REFRESH",
            )
        else:
            # Heads up box
            ...
            split = row.split(align=True)
            split.operator(
                operators.AIR_OT_automatic1111_load_controlnet_modules.bl_idname,
                text="",
                icon="FILE_REFRESH",
            )
            ...
            split = row.split(align=True)
            split.operator(
                operators.AIR_OT_automatic1111_load_controlnet_models.bl_idname,
                text="",
                icon="FILE_REFRESH",
            )
...
class AIR_PT_operation(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.enabled = (
            "Render Result" in bpy.data.images
            and bpy.data.images["Render Result"].has_data
        )
        ...
        if (
            props.do_autosave_before_images or props.do_autosave_after_images
        ) and not props.autosave_image_path:
            utils.label_multiline(
                layout, text="Please specify a path", icon="ERROR", width=width_guess
            )
...
class AIR_PT_upscale(bpy.types.Panel):
    ...
    @classmethod
    def does_backend_support_choosing_upscaler_model(cls, context):
        return utils.get_active_backend().supports_choosing_upscaler_model()

    @classmethod
    def does_backend_support_choosing_upscale_factor(cls, context):
        return utils.get_active_backend().supports_choosing_upscale_factor()
    ...
    def draw(self, context):
        ...
        if not AIR_PT_upscale.does_backend_support_upscaling(context):
            ...
            utils.label_multiline(
                box,
                text=f"Upscaling is not supported by {utils.sd_backend_formatted_name()}. If you'd like to upscale your image, switch to DreamStudio or Automatic1111 in AI Render's preferences.",
                icon="ERROR",
                width=width_guess,
            )
            return
        ...
        if not AIR_PT_upscale.is_upscaler_model_list_loaded(context):
            utils.label_multiline(
                layout,
                text="To get started upscaling, load the available upscaler models",
                icon="ERROR",
                width=width_guess,
            )
            layout.operator(
                operators.AIR_OT_automatic1111_load_upscaler_models.bl_idname,
                text="Load Upscaler Models",
                icon="FILE_REFRESH",
            )
            return
        ...
        if AIR_PT_upscale.does_backend_support_choosing_upscale_factor(context):
            row = layout.row()
            sub = row.column()
        ...
        if AIR_PT_upscale.does_backend_support_choosing_upscaler_model(context):
            row = layout.row()
            sub = row.column()
        ...
        row = box.row()
        row.label(
            text=f"Resulting image size: {utils.get_upscaled_width(scene)} x {utils.get_upscaled_height(scene)}"
        )

        # if the dimensions are too large, show message
        if not AIR_PT_upscale.are_upscaled_dimensions_small_enough(context):
            error_message = (
                "Upscaled dimensions are too large. Please decrease the scale factor."
                if AIR_PT_upscale.does_backend_support_choosing_upscale_factor(context)
                else "Upscaled dimensions are too large. Please decrease your scene's image size."
            )
            utils.label_multiline(
                layout,
                text=error_message,
        ...
        row.enabled = props.last_generated_image_filename != ""
        row.operator(
            operators.AIR_OT_upscale_last_sd_image.bl_idname, icon="FULLSCREEN_ENTER"
        )
...
class AIR_PT_inpaint(bpy.types.Panel):
    ...
    def poll(cls, context):
        return (
            utils.is_installation_valid()
            and context.scene.air_props.is_enabled
            and utils.get_active_backend().supports_inpainting()
        )
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.enabled = (
            props.last_generated_image_filename != "" and props.inpaint_mask_path != ""
        )
...
class AIR_PT_outpaint(bpy.types.Panel):
    ...
    def poll(cls, context):
        return (
            utils.is_installation_valid()
            and context.scene.air_props.is_enabled
            and utils.get_active_backend().supports_outpainting()
        )
    ...
    def draw(self, context):
        ...
        if is_animation_enabled_button_enabled:
            num_frames = math.floor(
                ((scene.frame_end - scene.frame_start) / scene.frame_step) + 1
            )
        ...
        row.operator(
            operators.AIR_OT_render_animation.bl_idname,
            icon="RENDER_ANIMATION",
            text=render_animation_text,
        )
        ...
        if round(props.image_similarity, 2) < 0.7 and not props.close_animation_tips:
            ...
            utils.label_multiline(
                box,
                text='For more stable animations, consider using ControlNet (locally) or increasing "Image Similarity" to at least 0.7',
                width=width_guess,
            )

            row = box.row()
            row.operator("wm.url_open", text="Get Animation Tips", icon="URL").url = (
                config.ANIMATION_TIPS_URL
            )
