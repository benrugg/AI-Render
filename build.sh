...
filename="dist/Stable-Diffusion-Render-v$version_str_for_file.zip"

# create a zip file using git, so we can create a top-level directory
# in the zip, and so it ignores all .gitignore patterns
cd ./src
git archive HEAD --prefix=Stable-Diffusion-Render/ --format=zip -o ../${filename}
