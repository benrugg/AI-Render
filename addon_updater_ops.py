...
updater.addon = "ai_render"
updater.addon_nice_name = "AI Render"
...
class AddonUpdaterInstallPopup(bpy.types.Operator):
    ...
    bl_label = f"Update {updater.addon_nice_name}"
...
class AddonUpdaterCheckNow(bpy.types.Operator):
    bl_label = f"Check now for {updater.addon_nice_name} update"
    bl_idname = updater.addon + ".updater_check_now"
    bl_description = f"Check now for an update to the {updater.addon_nice_name} add-on"
    bl_options = {'REGISTER', 'INTERNAL'}
...
class AddonUpdaterUpdateNow(bpy.types.Operator):
    bl_label = f"Update {updater.addon_nice_name} add-on now"
    bl_idname = updater.addon + ".updater_update_now"
    bl_description = f"Update to the latest version of the {updater.addon_nice_name} add-on"
    bl_options = {'REGISTER', 'INTERNAL'}
...
class AddonUpdaterUpdateTarget(bpy.types.Operator):
    bl_label = f"{updater.addon_nice_name} version target"
    bl_idname = updater.addon + ".updater_update_target"
    bl_description = f"Install a targeted version of the {updater.addon_nice_name} add-on"
    bl_options = {'REGISTER', 'INTERNAL'}
...
def updater_run_install_popup_handler(scene):
    ...
    if "ignore" in updater.json and updater.json["ignore"]:
        ...
        if ver_tuple < updater.current_version:
            ...
            updater.print_verbose(f"{updater.addon_nice_name} updater: appears user updated, clearing flag")
            updater.json_reset_restore()
            return
    atr = AddonUpdaterInstallPopup.bl_idname.split(".")
    getattr(getattr(bpy.ops, atr[0]), atr[1])('INVOKE_DEFAULT')
...
#         split.enabled = False
#         split.scale_y = 2
#         split.operator(AddonUpdaterCheckNow.bl_idname, text=f"{updater.addon_nice_name} is up to date")
#         split = sub_col.split(align=True)
#         split.scale_y = 2
#         split.operator(AddonUpdaterCheckNow.bl_idname,
#                        text="", icon="FILE_REFRESH")
...
def update_settings_ui_condensed(self, context, element=None):
    ...
    row = element.row()
    row.alignment = 'LEFT'
    row.prop(settings, 'updater_expanded_in_preferences_panel', text="Updates", icon="TRIA_DOWN" if settings.updater_expanded_in_preferences_panel else "TRIA_RIGHT", emboss=False)

    if settings.updater_expanded_in_preferences_panel:
        row = element.row()
...
def register(bl_info):
    ...
    updater.backup_ignore_patterns = [".git", "__pycache__"]
    ...
    updater.overwrite_patterns = ["*"] # ["*.png", "*.jpg", "*.md", "*.txt", "*.sh", "*.blend", ".editorconfig"]
